...
export default class ClassificationEngine {
   ...
   */
  static classifyMember(memberData: any, settings: any = null) {
    ...
    switch (membershipType) {
      ...
      case 'Wing': {
        const wingOccupation = (financial?.occupationType || '').toLowerCase();
        const wingSalary     = financial?.salary || 0;
        const wSettings      = rules?.wing || {};
        const wTypeName      = String(wing?.wingType || '').toLowerCase();
        
        subType = wing?.wingType || financial?.occupationType || 'General';
   ...
   * Determine cluster based on sector
   */
  static determineCluster(sector: string) {
    const urbanSectors = [
   ...
   */
  static classifySalaryBased(financial: any, rules: any, manualFinancial: any = null) {
   ...
   */
  static classifyNonSalary(financial: any, rules: any) {
   ...
   */
  static classifyBusiness(financial: any, rules: any) {
   ...
   */
  static classifyInvestor(financial: any, rules: any) {
   ...
   */
  static calculateIncomeTax(salary: number, rules: any = null) {
   ...
   */
  static calculatePension(salary: number, rules: any = null) {
   ...
   */
  static calculateNetSalary(salary: number, rules: any = null) {
   ...
   */
  static autoClassifyAndCalculate(memberData: any, settings: any = null) {
    ...
    const netSalaryData = this.calculateNetSalary(salary, settings?.contributionRules);
