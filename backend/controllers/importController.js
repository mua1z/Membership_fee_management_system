...
function mapExcelRowToMember(rawRow) {
  ...
  if (!membershipType && categoryName) {
    ...
    if (c.includes('wing')) membershipType = 'Wing Members';
    else if (c.includes('employee')) membershipType = 'Salary-Based Members';
    else if (c.includes('student')) membershipType = 'Student Members';
    else if (c.includes('investor')) membershipType = 'Investor Members';
    else if (c.includes('enterprise') || c.includes('business')) membershipType = 'Business Members';
    else if (c.includes('farmer') || c.includes('resident')) membershipType = 'Non-Salary Members';
  }

  if (!membershipType) {
    if (salary > 0) membershipType = 'Salary-Based Members';
    else if (capital > 0) membershipType = 'Investor Members';
    else if (businessType) membershipType = 'Business Members';
    else membershipType = 'Non-Salary Members';
  }
  ...
  return {
    ...
    financial: {
      salary:          membershipType === 'Salary-Based Members' || membershipType === 'Wing Members' ? salary : 0,
      employmentType:  getVal(['Employment Type', 'EmploymentType', 'የቅጥር ሁኔታ']) || 'Private',
      currency:        'ETB',
      businessType,
      income:          membershipType === 'Business Members' ? (Number(getVal(['Income', 'ገቢ'])) || salary) : 0,
      capital:         membershipType === 'Investor Members' ? (Number(getVal(['Capital', 'ካፒታል', 'Capital / ካፒታል'])) || salary) : 0,
